...
def_start = code.find("def process_file(file_path):")
def_end = code.find("def clean_all_eligible():")

new_def = """def process_file(file_path):
...
with open("scripts/clean_video_script.py", "w") as f:
    f.write(code[:def_start] + new_def + code[def_end:])
