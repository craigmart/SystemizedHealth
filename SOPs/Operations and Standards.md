...
All content blueprints and local media assets adhere strictly to the centralized 80 Johnny Decimal framework. This architectural system expands dynamically around clinical concepts rather than rigid calendar timelines.
Standard Taxonomic Notation: 80.V[Hierarchy]
80: The primary Johnny Decimal classification designated for the Systemized Health ecosystem.
V: Denotes a core video asset within the clinical production pipeline.


[Hierarchy]: The non-linear, alternating alphanumeric sequence tracking subsequent conceptual developments (e.g., 1A1, B2A3).
...
The Hook: An immediate, high-engagement introduction confronting a widespread physical pain point. This segment must reject any formulaic &apos;not X, but Y&apos; structural contradictions.
Reframe Statement: A decisive declaration driving immediate audience resonance, shifting the focus from structural hardware limitations (muscles or joints) to biological software mechanisms (neurology) while steering clear of cliché interrogative phrasing.
Story: A real-world patient narrative confirming the conceptual paradigm transformation.
The Teach: The foundational breakdown of the underlying biological science and somatic mechanics.
Action Step: An executable somatic assessment or clinical movement protocol.
Engagement Integration: A targeted directive designed to smoothly channel viewer attention directly into the authorized conversion architecture.
...
Subject Nodes (The Parent): Create a card for the biological concept and assign it the master code 80.[Year][Week]-N1.
Asset Cards (The Child): Assign each recorded clip a sequential three-digit number matching the raw file on the hard drive. An example is 80.2627-N1-001 for a clip showing slouching at a desk.
...
The Audio Dump: Record a detailed audio dictation including research asks and editorial notes and drop the transcript into a fresh Gemini chat with NotebookLM attached.

Cohesive Arc Generation: The editor refines the rambling audio transcript into a cohesive script loosely following the 6-part Systemized Outline Structure while strictly maintaining the creator's vibe as determined by the rough transcript and Writing guidelines and Text DNA.
Final Edit & Filing: The creator edits the script, locks it in, and moves the final script directly into NotebookLM.
A-Roll Execution: Film the long video A-roll as a talking head positioned on the right side of the frame. Film the 2 to 4 shorts branching off this topic. Script is used during filming.
A-Roll Transcription: Generate a transcript directly from the raw A-roll footage. This transcript becomes the gold standard for making slides and main cards.  The transcript will be uploaded to Notebook LM.
Slide Generation: Use NotebookLM to generate simple supporting slides based on the new A-roll transcript. Slides must have a solid black background, pure white text and diagrams, include all research citations, and be offset to the left.
...
The Playlist Framework (Systemized OS) To force the algorithm to adopt a hub-and-spoke model, all channel content must be strictly organized into these five playlists to mirror the biological architecture:
Start Here: The Systemized OS Installation (Onboarding & core philosophy)
Level 1: Foundational (FMR) (Fuel, Move, Rest)
Level 2: Internal (TLC) (Think, Learn, Connect)
Level 3: External (POP) (Play, Organize, Purpose)
The Clinical Lab (Deep Dives) (Neurological breakdowns & authority content)
Title Guardrails Titles must be a "decisive declaration" or "clinical directive." They must shift the focus from a standard physical problem to a biological software mechanism, framing health failures as a "system glitch" requiring a structured update. Never use interrogative phrasing (questions) or standard influencer tropes.
Thumbnail Design Thumbnails must move away from generic fitness imagery. They should incorporate visual graphics mapping the three-tiered Systemized OS structure to instantly highlight which specific module of the operating system (e.g., Fuel, Move, Rest) is being installed in that video.
The Hub-and-Spoke Description Strategy Every video description must immediately identify where the topic sits within the strict biological hierarchy of the Systemized OS (Level 1, 2, or 3). The description must explicitly remind the viewer that executing higher levels (like Level 3 external goals or Level 2 cognitive tasks) is impossible without a stable physical baseline (Level 1 FMR).
Proprietary Tagging Generic, highly-saturated lifestyle tags (e.g., #motivation, #weightloss, #dopamine) are strictly prohibited to avoid algorithmic association with the self-help niche. Tags must exclusively use proprietary, clinical terminology (e.g., Systemized Health, Systemized OS, Biological Operating System, FMR Baseline, Autonomic Regulation, TLC Architecture).
